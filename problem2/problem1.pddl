...
;; alpha_bot_small, robot that process alpha artifact with capacity one;
;; beta_bot_medium, robot that process beta artifact with capacity two. 

(define (problem interplanetary_museum_vault_problem2)
  (:domain interplanetary_museum_vault)

  (:objects
    alpha_bot_small - alpha_robot
    beta_bot_medium - beta_robot
  ...
  (:init
    ;; Robot locations
    (at alpha_bot_small entrance)
    (at beta_bot_medium entrance)
    
    ;; Robot sealing
    (not (sealed alpha_bot_small))
    (not (sealed beta_bot_medium))
    
    ;; Robot capacities
    (capacity_level_0 alpha_bot_small)
    (capacity_level_0 beta_bot_medium)
    (max_capacity_1 alpha_bot_small)  
    (max_capacity_2 beta_bot_medium)
    (has_capacity alpha_bot_small)
    (has_capacity beta_bot_medium)
